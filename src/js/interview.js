...
function getOnce() { // 读取历史
  const lastnum = localStorage.getItem('lastnum');
  if (lastnum !== null) {
    return lastnum;
  }
  return 1;
...
function groupSwitch(num) {
  const grouparr = ['前端', '后台', '安卓', 'IOS', '机器学习'];
...
$('#input-4').on('blur', function () { // 检测时间
  const startTime = $('#input-3').val().replace('-', '');
  const endTime = $('#input-4').val().replace('-', '');
...
$('.input-base').on('focus', function () { // 聚焦恢复
  const sayarr = ['最大面试人数', '选择日期', '开始时间', '结束时间'];
...
$('.interview-top .icon-box').click(() => { // 刷新按钮
  $(`[groups=${groupId}]`).trigger('click');
});

$('.output-interview').click(() => { // 点击确认发布面试
  const inputValueArr = [];
